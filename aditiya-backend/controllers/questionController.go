...
func CalculateScore(c *gin.Context) {
	...
	if err := config.DB.Where("packet_id = ?", input.PacketID).Find(&dbQuestions).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	for _, userAnswer := range input.Answers {
		var question models.Question
		// Ambil pertanyaan berdasarkan QuestionID dari database
		if err := config.DB.Where("id = ?", userAnswer.QuestionID).First(&question).Error; err == nil {
			// Cek jika jawaban kosong (tidak dijawab)
			if userAnswer.Answer == "" {
				unansweredCount++ // Jika tidak dijawab, tambahkan ke unansweredCount
			} else {
				// Jika jawaban benar
	...
	score := 0
	if totalQuestions > 0 {
		// Menghitung skor persentase dan mengonversinya menjadi integer
